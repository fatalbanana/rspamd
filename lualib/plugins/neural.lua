  ...
  if #params.ham_vec + #params.spam_vec < params.rule.train.max_trains / 2 then
    ...
    local function ann_trained(err, data)
      ...
      if err then
        ...
        params.set.ann.redis_key = new_ann_key(params.rule, params.set, version, params.settings)
...
return {
  ...
  redis_lua_script_vectors_len_id = redis_lua_script_vectors_len_id,
  redis_maybe_invalidate_id = redis_maybe_invalidate_id,
  redis_maybe_lock_id = redis_maybe_lock_id,
  redis_params = redis_params,
  redis_save_unlock_id = redis_save_unlock_id,
  settings = settings,
